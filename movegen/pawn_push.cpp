...
void generate_pawn_push_moves(T& moves, Board& board, detail_t& detail) {
  ...
  bitboard_t rook_checking_squares = attack<rook>(detail.opponent_king_square, board.bitboards[none] & ~(board.bitboards[color_pawn] & prepromotion_rank));
  ...
  if constexpr (color == white) {
    pushable = (
      ...
      (rook_pinned_pawns & ~(board.bitboards[none] << 8) & (detail.evasion_targets << 8) & (rook_ray[detail.color_king_square] << 8))
    );
    doublepushable = (
      (free_pawns & ~(board.bitboards[none] << 8) & ~(board.bitboards[none] << 16) & (detail.evasion_targets << 16) & rank_2) |
      (rook_pinned_pawns & ~(board.bitboards[none] << 8) & ~(board.bitboards[none] << 16) & (detail.evasion_targets << 16) & rank_2 & (rook_ray[detail.color_king_square] << 16))
    );
    ...
    doublepushable = (
      ...
      (rook_pinned_pawns & ~(board.bitboards[none] >> 8) & ~(board.bitboards[none] >> 16) & (detail.evasion_targets >> 16) & rank_7 & (rook_ray[detail.color_king_square] >> 16))
    );
    promoting = free_pawns & ~(board.bitboards[none] >> 8) & (detail.evasion_targets >> 8) & rank_2;
  };
  // refine definitions by discoverability
  ...
  // define the final types of pawns
  ...
  if constexpr (std::is_same_v<T, move_stack_t>) {
    ...
    while (rook_promoting_pawns) {
      ...
      square_t to = from + push_offset;
      moves.push(move(from, to, color_pawn, color_rook, none, 0, 0, 0, 1, (bitboard(from) & discoverable) || (bitboard(to) & rook_checking_squares)));
    };
    while (queen_promoting_pawns) {
      square_t from = pop_lsb(queen_promoting_pawns);
      square_t to = from + push_offset;
      moves.push(move(from, to, color_pawn, color_queen, none, 0, 0, 0, 1, (bitboard(from) & discoverable) || (bitboard(to) & (detail.bishop_checking_squares | rook_checking_squares))));
    };
  } else {
