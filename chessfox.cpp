...
int main() {
  ...
  time = timing::nanoseconds();
  depth = 7;
  count = board.perft(depth, true);
  cout << endl << "\tPerft " << depth << " of " << endl;
  cout << "\t" << board.fen() << endl;
  cout << "\tTotal:\t" << count << "\t\tMN/s:\t" << 1000 * ((float)count) / ((float)(timing::nanoseconds() - time)) << endl;
  cout << "\tIn " << (float)(timing::nanoseconds() - time) / 1000000000 << " seconds." << endl;
  cout << endl;
