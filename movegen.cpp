...
#ifndef __GEN__MODULE__
#define __GEN__MODULE__
...
namespace movegen {
  struct info_t {
    ...
    void update(Board& board) {
      ...
      bitboard_t king_rook_attack = attack<piece::rook>(this->king_square, board.bitboards[piece::none]);
        ...
        switch (board.pieces[this->checker_square]) {
          case opponent_queen:
            if (king_bishop_attack & board.bitboards[opponent_queen]) {
              this->evasion_target = bitboard(this->checker_square) | (king_bishop_attack & attack<piece::bishop>(this->checker_square, board.bitboards[color::none]));
            } else if (king_rook_attack & board.bitboards[opponent_queen]) {
              this->evasion_target = bitboard(this->checker_square) | (king_rook_attack & attack<piece::rook>(this->checker_square, board.bitboards[color::none]));
            };
            break;
          case opponent_rook:
            this->evasion_target = bitboard(this->checker_square) | (king_rook_attack & attack<piece::rook>(this->checker_square, board.bitboards[color::none]));
      ...
      bishop_attackers = (board.bitboards[bishop] | board.bitboards[queen]) & attack_ray::bishop[this->opponent_king_square];
      while (bishop_attackers) {
        square_t square = pop_lsb(bishop_attackers);
        this->bishop_discoverable |= attack<bishop>(square, board.bitboards[color::none]) & this->bishop_checking_squares;
      };
      this->rook_discoverable = bitboard::none;
      rook_attackers = (board.bitboards[rook] | board.bitboards[queen]) & attack_ray::rook[this->opponent_king_square];
      while (rook_attackers) {
        square_t square = pop_lsb(rook_attackers);
        this->rook_discoverable |= attack<rook>(square, board.bitboards[color::none]) & this->rook_checking_squares;
      };
    };
  };

  // generate all moves of a given generation type
  ...
  T generate(Board& board) {
    ...
    if (popcount(info.checkers) > 1) {
      generate_king<color, gen>(moves, board, info);
      return moves;
    };
    generate_castling<color, gen>(moves, board, info);
    generate_king<color, gen>(moves, board, info);
    generate_queen<color, gen>(moves, board, info);
    generate_rook<color, gen>(moves, board, info);
    generate_bishop<color, gen>(moves, board, info);
    generate_knight<color, gen>(moves, board, info);
    generate_pawn<color, gen>(moves, board, info);
    return moves;
  };

  // generate castling moves
  template <color_t color, gen_t gen, typename T>
  void generate_castling(T& moves, Board& board, info_t& info) {
    if constexpr (color == color::white) {
      bool castle_king = (
        ...
        (((gen & check) && (bitboard::f1 & info.rook_checking_squares_castling)) || ((gen & quiet) && !(bitboard::f1 & info.rook_checking_squares_castling)))
      );
      bool castle_queen = (
        ...
        (((gen & check) && (bitboard::d1 & info.rook_checking_squares_castling)) || ((gen & quiet) && !(bitboard::d1 & info.rook_checking_squares_castling)))
      );
      if constexpr (std::is_same_v<T, move_stack_t>) {
        if (castle_king)
      ...
      bool castle_king = (
        ...
        (((gen & check) && (bitboard::f8 & info.rook_checking_squares_castling)) || ((gen & quiet) && !(bitboard::f8 & info.rook_checking_squares_castling)))
      );
      bool castle_queen = (
        ...
        (((gen & check) && (bitboard::d8 & info.rook_checking_squares_castling)) || ((gen & quiet) && !(bitboard::d8 & info.rook_checking_squares_castling)))
      );
      if constexpr (std::is_same_v<T, move_stack_t>) {
        if (castle_king)
  ...
  // generate king moves
  template <color_t color, gen_t gen, typename T>
  void generate_king(T& moves, Board& board, info_t& info) {
    constexpr color_t opponent = color::compiletime::opponent(color);
    constexpr piece_t king = piece::compiletime::to_color(piece::king, color);
    ...
    bitboard_t possible_to = target & attack<piece::king>(info.king_square) & ~board.bitboards[color] & ~info.unsafe_king_squares;
    if constexpr (std::is_same_v<T, move_stack_t>) {
      while (possible_to) {
  ...
  template <color_t color, gen_t gen, typename T>
  void generate_queen(T& moves, Board& board, info_t& info) {
    constexpr color_t opponent = color::compiletime::opponent(color);
    constexpr piece_t queen = piece::compiletime::to_color(piece::queen, color);
    ...
    while (free_queens) {
      ...
      bitboard_t possible_to = attack<piece::queen>(from, board.bitboards[color::none]) & ~board.bitboards[color] & target;
      if constexpr (std::is_same_v<T, move_stack_t>) {
        while (possible_to) {
          square_t to = pop_lsb(possible_to);
          moves.push(move::move(from, to, queen, queen, board.pieces[to], false, false, false, false, bitboard(to) & checking_squares));
  ...
  // generate rook moves
  template <color_t color, gen_t gen, typename T>
  void generate_rook(T& moves, Board& board, info_t& info) {
    constexpr color_t opponent = color::compiletime::opponent(color);
    constexpr piece_t rook = piece::compiletime::to_color(piece::rook, color);
    ...
    while (non_discoverable_free_rooks) {
      ...
      bitboard_t possible_to = attack<rook>(from, board.bitboards[color::none]) & ~board.bitboards[color] & non_discoverable_target;
      if constexpr (std::is_same_v<T, move_stack_t>) {
        while (possible_to) {
          square_t to = pop_lsb(possible_to);
          moves.push(move::move(from, to, rook, rook, board.pieces[to], false, false, false, false, bitboard(to) & checking_squares));
    ...
    while (bishop_discoverable_free_rooks) {
      ...
      bitboard_t possible_to = attack<rook>(from, board.bitboards[color::none]) & ~board.bitboards[color] & bishop_discoverable_target;
      if constexpr (std::is_same_v<T, move_stack_t>) {
        while (possible_to) {
          square_t to = pop_lsb(possible_to);
          moves.push(move::move(from, to, rook, rook, board.pieces[to], false, false, false, false, true));
  ...
  template <color_t color, gen_t gen, typename T>
  void generate_bishop(T& moves, Board& board, info_t& info) {
    constexpr color_t opponent = color::compiletime::opponent(color);
    constexpr piece_t bishop = piece::compiletime::to_color(piece::bishop, color);
    ...
    while (non_discoverable_free_bishops) {
      ...
      bitboard_t possible_to = attack<bishop>(from, board.bitboards[color::none]) & ~board.bitboards[color] & non_discoverable_target;
      if constexpr (std::is_same_v<T, move_stack_t>) {
        while (possible_to) {
          square_t to = pop_lsb(possible_to);
          moves.push(move::move(from, to, bishop, bishop, board.pieces[to], false, false, false, false, bitboard(to) & checking_squares));
    ...
    while (rook_discoverable_free_bishops) {
      ...
      bitboard_t possible_to = attack<bishop>(from, board.bitboards[color::none]) & ~board.bitboards[color] & rook_discoverable_target;
      if constexpr (std::is_same_v<T, move_stack_t>) {
        while (possible_to) {
          square_t to = pop_lsb(possible_to);
          moves.push(move::move(from, to, bishop, bishop, board.pieces[to], false, false, false, false, true));
  ...
  template <color_t color, gen_t gen, typename T>
  void generate_knight(T& moves, Board& board, info_t& info) {
    constexpr color_t opponent = color::compiletime::opponent(color);
    constexpr piece_t knight = piece::compiletime::to_color(piece::knight, color);
    bitboard_t checking_squares = attack<knight>(info.opponent_king_square);
    bitboard_t non_discoverable_target = bitboard::none;
    ...
    while (non_discoverable_free_knights) {
      ...
      bitboard_t possible_to = attack<knight>(from) & ~board.bitboards[color] & non_discoverable_target;
      if constexpr (std::is_same_v<T, move_stack_t>) {
        while (possible_to) {
          square_t to = pop_lsb(possible_to);
    ...
    while (discoverable_free_knights) {
      ...
      bitboard_t possible_to = attack<knight>(from) & ~board.bitboards[color] & discoverable_target;
      if constexpr (std::is_same_v<T, move_stack_t>) {
        while (possible_to) {
          square_t to = pop_lsb(possible_to);
  ...
  // generate pawn moves
  template <color_t color, gen_t gen, typename T>
  void generate_pawn(T& moves, Board& board, info_t& info) {
    generate_pawn_captures<color, gen>(moves, board, info);
    generate_pawn_pushes<color, gen>(moves, board, info);
  };

  // generate pawn captures
  template <color_t color, gen_t gen, typename T>
  void generate_pawn_captures(T& moves, Board& board, info_t& info) {
    generate_pawn_enpassant<color, gen>(moves, board, info);
    constexpr color_t opponent = color::compiletime::opponent(color);
    ...
    bitboard_t pawn_checking_squares = attack<opponent_pawn>(info.opponent_king_square);
    bitboard_t knight_checking_squares = attack<knight>(info.opponent_king_square);
    bitboard_t bishop_checking_squares_no_pawns = attack<bishop>(info.opponent_king_square, board.bitboards[piece::none] & ~board.bitboards[pawn]);
    ...
    while (discoverable_promoting_free_pawns) {
      ...
      bitboard_t possible_to = attack<pawn>(from) & board.bitboards[opponent] & discoverable_target;
      if constexpr (std::is_same_v<T, move_stack_t>) {
        while (possible_to) {
          square_t to = pop_lsb(possible_to);
    ...
    while (non_discoverable_promoting_free_pawns) {
      ...
      bitboard_t possible_to_knight = attack<pawn>(from) & board.bitboards[opponent] & non_discoverable_knight_promoting_target;
      bitboard_t possible_to_bishop = attack<pawn>(from) & board.bitboards[opponent] & non_discoverable_bishop_promoting_target;
      bitboard_t possible_to_rook = attack<pawn>(from) & board.bitboards[opponent] & non_discoverable_rook_promoting_target;
      bitboard_t possible_to_queen = attack<pawn>(from) & board.bitboards[opponent] & non_discoverable_queen_promoting_target;
    ...
    while (non_discoverable_not_promoting_free_pawns) {
      ...
      bitboard_t possible_to = attack<pawn>(from) & board.bitboards[opponent] & non_discoverable_target;
      if constexpr (std::is_same_v<T, move_stack_t>) {
        while (possible_to) {
          square_t to = pop_lsb(possible_to);
  ...
  template <color_t color, gen_t gen, typename T>
  void generate_pawn_enpassant(T& moves, Board& board, info_t& info) {
    constexpr piece_t pawn = piece::compiletime::to_color(piece::pawn, color);
    constexpr piece_t rook = piece::compiletime::to_color(piece::rook, color);
    constexpr piece_t queen = piece::compiletime::to_color(piece::queen, color);
    constexpr color_t opponent = color::compiletime::opponent(color);
    ...
    bool is_always_check = (info.bishop_discoverable & bitboard(board.enpassant + enpassant_offset)) || (board.bitboards[opponent_king] & attack<pawn>(board.enpassant));
  ...
  // generate pawn pushes
  template <color_t color, gen_t gen, typename T>
  void generate_pawn_pushes(T& moves, Board& board, info_t& info) {
    constexpr color_t opponent = color::compiletime::opponent(color);
    constexpr piece_t pawn = piece::compiletime::to_color(piece::pawn, color);
    constexpr piece_t knight = piece::compiletime::to_color(piece::knight, color);
    constexpr piece_t bishop = piece::compiletime::to_color(piece::bishop, color);
